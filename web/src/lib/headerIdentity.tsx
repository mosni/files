  ...
  return (
    <>
      ...
      {/* Review session 052: D-168's fallback renders the raw `sub` whenever no name was captured, and
          ...
          away from. `display: inline-block` and `maxWidth` are both load-bearing: `text-overflow` does
          nothing on an inline box with no width constraint. The full value stays in `title`. Nothing here
          parses the sub (invariant 6) - the three sibling sites (both ShareDialog lists, PreviewCard's
          byline) get the identical treatment. */}
      <span
        data-identity-name=""
        title={name}
        style={{
